...
test("setup phases run in the approved order and seed is opt-in", async () => {
  ...
  assert.deepEqual(order, ["validate", "pages", "databases", "relations", "views", "docs", "seed", "report"]);
});
...
test("view manual actions do not fail core setup", async () => {
  ...
  const result = await orchestrate({
    ...
    context: {},
    actions: { validate: noop, pages: noop, databases: noop, relations: noop, views: async () => ({ created: 0, reused: 0, manualActions: ["view manual"] }), docs: noop, seed: noop, report: noop },
  });
